...
<Application xmlns="http://schemas.microsoft.com/windowsazure">
  ...
  mobile.createService = function (options, callback) {
    ...
    if (options.sqlServer) {
      ...
      serverSpec = {
        "ProvisioningParameters": {
          "AdministratorLogin": "tjanczuk",
          "AdministratorLoginPassword": "Pilsner4*",
          "Location": "West US"
        },
    ...
    var dbSpec = {
      ...
      // "ProvisioningConfigParameters": null,
      // "DeploymentParameters": null,
      "Version": "1.0",
      "Name": dbRefName,
      // "PostProvisionedName": null,
      ...
      "InternalResources": {
        "ZumoMobileService": {
          ...
          "ProvisioningConfigParameters": {
            ...
            "AdministratorLogin": "tjanczuk", // get
            "AdministratorLoginPassword": "Pilsner4*" // get
          },
          // "DeploymentParameters": null,
    ...
    var payload = createMobileServiceApplicationTemplate
      .replace('##name##', options.servicename)
          ...
          channel.GET(function (error, body, res) {
            ...
            if (log.format().json) {
              ...
              flat.Resources.forEach(function (resource) {
                logger(resource.TypeView + (resource.Name ? ' (' + resource.Name + ')' : '') + ' state: ' + resource.State);
                if (resource.Error) {
...
mobile.command('get [servicename]')
  ...
  mobile.command('create [servicename]')
        .usage('[options] [servicename]')
        .whiteListPowershell()
